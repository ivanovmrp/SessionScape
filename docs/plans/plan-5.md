...
- **Tasks**:
  ...
  2. Write failing component tests for practitioner and service create/edit/deactivate plus their use in selectors; implement the minimum catalog controls required by the ledger.
  ...
  5. Write failing unit and component tests for malformed records, dangling/inactive references, self-aware overlap rules, adjacent/cancelled/no-show cases, and outside-hours confirmation; implement blocking validation and the explicit override.
- **Test plan**:
  ...
  - Criteria 2–3: component integration tests cover catalog CRUD/deactivation, stable-ID appointment CRUD, every status and lifecycle transition, refill matching, confirmed deletion, generated anonymous-ID selection, explicit closed days, active-practitioner coverage, and dedicated availability editing.
