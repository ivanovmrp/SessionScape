...
- **Status**: in progress
- **Dependencies**: BL-001
- **Likely touched files**: `.github/workflows/ci.yml`, a temporary `lib/ci-failure.test.ts` removed before completion, `docs/plans/plan-2.md`; GitHub `main` branch-protection settings after the check context is verified
...
- **Tasks**:
  ...
  3. Delete only the sentinel and commit GREEN. After explicit authorization, push it and verify the latest `CI / quality` run remains visible and completes successfully. To prove supersession, push a branch update, wait until its run is active, then after explicit authorization push a clearly labeled empty superseding commit and verify the older run is cancelled while the latest run completes successfully.
