...
- **Tasks**:
  ...
  3. Delete only the sentinel and commit GREEN. After explicit authorization, push it and immediately push a clearly labeled empty test-trigger commit; verify the superseded run is cancelled while the latest `CI / quality` run remains visible and completes successfully.
