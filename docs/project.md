...
- **Structure**: App Router prototype in `app/`; typed synthetic scenarios in `lib/`; CI quality workflow in `.github/workflows/ci.yml`; product, architecture, privacy, and business evidence in `docs/`
...
- **Learnings**: A repository's first required workflow needs a bootstrap merge before its `main` event and app identity can be verified; prove cancellation with a temporary passing delay after the run is active (Plan 2)
